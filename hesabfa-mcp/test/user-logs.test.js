...
test('imports only the three session headers from a copied browser request', () => {
...
test('expired or unauthorized sessions cannot become empty activity reports or expose body text', async () => {
  for (const status of [401, 403]) await assert.rejects(
    ...
    error => error.code === 'USER_LOG_SESSION_EXPIRED_OR_FORBIDDEN' && !error.message.includes('test-cookie')
  );
});
